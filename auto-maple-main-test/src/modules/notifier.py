...
RUNE_IMAGE = cv2.imread('assets/rune.png', 0)
...
VIOLLETA = cv2.imread('assets/violetta.png', 0)
    ...
    def _main(self):
        ...
        while True:
            if config.enabled:
                ...
                # using the part of the frame
                # elite_frame = frame[height // 4: 3 * height // 4, width // 4: 3 * width // 4]

                # using whole frame
                elite_frame = frame
                elite = utils.multi_match(elite_frame, VIOLLETA, threshold=0.5)
                if len(elite) > 0:
                    self._alert('siren')
                ...
                if not config.bot.rune_active:
                    ...
                    rune = utils.multi_match(elite_frame, RUNE_IMAGE, threshold=0.5)
                    rune_start_time = now
